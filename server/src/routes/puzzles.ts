...
router.post('/:id/solve', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  const attempt = await prisma.puzzleAttempt.upsert({
    where: { userId_puzzleId: { userId: req.userId!, puzzleId: req.params.id } },
    update: { solved: true, movesUsed, timeSeconds, solvedAt: new Date() },
    create: { userId: req.userId!, puzzleId: req.params.id, solved: true, movesUsed, timeSeconds, solvedAt: new Date() },
  });
...
router.get('/:id/ranking', async (req, res: Response) => {
  const attempts = await prisma.puzzleAttempt.findMany({ where: { puzzleId: req.params.id, solved: true }, orderBy: [{ movesUsed: 'asc' }, { timeSeconds: 'asc' }], take: 20, include: { user: { select: { username: true, city: true } } } });
